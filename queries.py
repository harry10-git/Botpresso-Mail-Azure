...
def get_top_queries(service, site_url, start_date, end_date, dimensions, row_limit):
    request = {
...
if __name__ == "__main__":
    ...
    end_date = '2023-01-31'
    dimensions = ['query']
    row_limit = 3  # Fetch the top 3 queries

    try:
        service = initialize_gsc_service(service_account_json_key, scopes)
        response = get_top_queries(service, site_url, start_date, end_date, dimensions, row_limit)
        df_top_queries = process_response(response)

        # Create HTML content for email
        html_content = create_html_content(df_top_queries)
        ...
        subject = 'Top Performing Queries for Your Website'
